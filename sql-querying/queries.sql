...
SELECT app_name, price, reviews FROM analytics WHERE price BETWEEN 0.10::real AND 0.99::real ORDER BY reviews DESC LIMIT 10;
...
SELECT * FROM analytics WHERE price = (SELECT MAX(price) FROM analytics) LIMIT 1;
...
SELECT app_name, reviews, min_installs, (min_installs/reviews) AS proportion FROM analytics ORDER BY (min_installs/reviews) DESC LIMIT 1;

-- Find the name and rating of the top rated apps in each category, among apps that have been installed at least 50,000 times. Hint: Here you’ll probably need a subquery. --
